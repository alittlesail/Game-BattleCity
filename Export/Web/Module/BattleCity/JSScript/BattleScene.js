...
BattleCity.BattleScene = JavaScript.Class(ALittle.DisplayLayout, {
	...
	Show : async function(stage) {
		...
		for (let [row, sub_map] of this._sprite_map) {
			...
			for (let [col, sprite] of sub_map) {
				...
				if (sprite.col_index >= (4 - 1) * 4 + 1 && sprite.col_index <= (4) * 4) {
	...
	TryRemoveWall : function(row, col) {
		...
		if (sprite.col_index >= (1 - 1) * 4 + 1 && sprite.col_index <= (1) * 4) {
	...
	CanWalkByMap : function(row, col) {
		...
		return sprite.col_index >= (3 - 1) * 4 + 1 && sprite.col_index <= (3) * 4;
	},
	...
	BulletCollisionByMap : function(row, col, min_or_max, dir) {
		...
		if (sprite.col_index >= (2 - 1) * 4 + 1 && sprite.col_index <= (2) * 4) {
			return true;
		}
		if (sprite.col_index >= (3 - 1) * 4 + 1 && sprite.col_index <= (3) * 4) {
			return false;
		}
		if (sprite.col_index >= (4 - 1) * 4 + 1 && sprite.col_index <= (4) * 4) {
			return false;
		}
		if (sprite.col_index >= (5 - 1) * 4 + 1 && sprite.col_index <= (5) * 4) {
			return false;
		}
		if (sprite.col_index >= (7 - 1) * 4 + 1 && sprite.col_index <= (7) * 4) {
			return false;
		}
		if (sprite.col_index >= (6 - 1) * 4 + 1 && sprite.col_index <= (6) * 4) {
			for (let r = 12 * 4; r < 13 * 4; r += 1) {
				for (let c = 6 * 4; c < 7 * 4; c += 1) {
					this.SetTileShow(r, c, 7);
				}
			}
			if (this._castle_explosion === undefined) {
	...
	GenerateItem : function() {
		...
		let item_type = ALittle.Math_RandomInt(1, 6);
	...
	Start : function() {
		if (g_GCenter.player_count >= 1) {
			...
			this._player_1.StartBorn(12 * 4, 4 * 4, g_GCenter.player1_data.level, 1, 0.08);
		}
		if (g_GCenter.player_count >= 2) {
			this._entity_container.AddChild(this._player_2);
			this._player_2.StartBorn(12 * 4, 8 * 4, g_GCenter.player2_data.level, 1, 0.08);
		}
	},
	HandleFrame : function(frame_time) {
		...
		for (let index = 1; index <= ___OBJECT_5.length; ++index) {
			...
			if (player !== undefined) {
				...
				} else if (child.sprite.col_index === 3) {
					this._iron_flash = false;
					this._iron_flash_count = 0;
					this._iron_flash_frame = 0;
					let brush_type = 2;
				...
				} else if (child.sprite.col_index === 6) {
					player.StartShield();
				} else if (child.sprite.col_index === 2) {
